...
import { Client, Project, ClientFeedback, SatisfactionLevel, ProjectStatus, Contract, Transaction, TransactionType, Profile, Package, QRCodeRecord } from '../types';
import { FolderKanbanIcon, getProjectStatusColor, ClockIcon, MapPinIcon, CalendarIcon, StarIcon, SmileIcon, DownloadIcon, UsersIcon, FileTextIcon, HistoryIcon, HomeIcon, DollarSignIcon, TrendingUpIcon, TrendingDownIcon, PrinterIcon, QrCodeIcon } from '../constants';
...
const ClientPortal: React.FC<ClientPortalProps> = ({ accessId, clients, projects, contracts, transactions, setClientFeedback, showNotification, profile, packages, qrCodes }) => {
    ...
    const stats = useMemo(() => {
        ...
        const upcomingProjects = clientProjects.filter(p => new Date(p.date) >= new Date() && p.status !== ProjectStatus.COMPLETED);
        return { totalValue, totalPaid, remaining, upcomingProjects };
    }, [clientProjects]);
    
     useEffect(() => {
        if (invoiceToView && showDigitalSignature) {
    ...
    }, [invoiceToView, showDigitalSignature, client, transactions, qrCodes]);
    ...
    const tabs = [
        ...
        { id: 'contracts', label: 'Kontrak', icon: FileTextIcon },
        { id: 'payments', label: 'Pembayaran', icon: HistoryIcon },
    ];
            ...
            <div id="invoice-content" className="p-1">
                <div className="printable-content bg-white p-8 font-sans text-gray-800">
                    ...
                    <table className="w-full text-left mb-8">
                        ...
                        <tbody>
                            <tr><td className="p-3"><p className="font-semibold">{project.packageName}</p><p className="text-xs text-gray-500">{selectedPackage?.description}</p></td><td className="p-3 text-center">1</td><td className="p-3 text-right">{formatCurrency(selectedPackage?.price || 0)}</td><td className="p-3 text-right">{formatCurrency(selectedPackage?.price || 0)}</td></tr>
    ...
    const renderTabContent = () => {
        switch (activeTab) {
            ...
            case 'projects':
                return <ProjectsTab projects={clientProjects} onProjectClick={setSelectedProject} />;
            case 'contracts':
        ...
        <div className="min-h-screen bg-brand-bg text-brand-text-primary p-4 sm:p-6 lg:p-8">
            <div className="max-w-5xl mx-auto">
                ...
                <Modal isOpen={!!selectedProject} onClose={() => setSelectedProject(null)} title={`Detail Proyek: ${selectedProject?.projectName}`} size="3xl">
                    {selectedProject && <ProjectDetailModal project={selectedProject} transactions={transactions} />}
                </Modal>
...
const ProjectsTab: React.FC<{projects: Project[], onProjectClick: (project: Project) => void}> = ({ projects, onProjectClick }) => (
     <div className="bg-brand-surface p-6 rounded-2xl shadow-lg border border-brand-border">
            ...
            {projects.map(p => (
                <div key={p.id} onClick={() => onProjectClick(p)} className="p-4 bg-brand-bg rounded-xl border border-brand-border cursor-pointer hover:border-brand-accent">
                    <div className="flex justify-between items-center">
                        ...
                        <span className={`px-2 py-1 text-xs font-medium rounded-full`} style={{ backgroundColor: `${getProjectStatusColor(p.status)}20`, color: getProjectStatusColor(p.status)}}>{p.subStatus ? `${p.status}: ${p.subStatus}` : p.status}</span>
                    </div>
                    <div className="w-full bg-brand-input rounded-full h-2.5 my-3"><div className="h-2.5 rounded-full" style={{ width: `${p.progress}%`, backgroundColor: getProjectStatusColor(p.status) }}></div></div>
...
const ProjectDetailModal: React.FC<{project: Project, transactions: Transaction[]}> = ({ project, transactions }) => {
    const projectPayments = transactions.filter(t => t.projectId === project.id && t.type === TransactionType.INCOME);
    return (
        <div className="space-y-4 text-sm">
            <div><p className="font-semibold text-brand-text-secondary">Status & Progress</p><div className="w-full bg-brand-input rounded-full h-2.5 mt-2"><div className="h-2.5 rounded-full" style={{ width: `${project.progress}%`, backgroundColor: getProjectStatusColor(project.status) }}></div></div></div>
