...
import { TeamMember, Project, TeamProjectPayment, FreelancerFeedback, Revision, RevisionStatus, PerformanceNoteType, TeamPaymentRecord, RewardLedgerEntry, PerformanceNote } from '../types';
import Modal from './Modal';
import { CalendarIcon, CreditCardIcon, MessageSquareIcon, ClockIcon, UsersIcon, FileTextIcon, MapPinIcon, HomeIcon, FolderKanbanIcon, StarIcon, DollarSignIcon, AlertCircleIcon } from '../constants';
...
interface FreelancerPortalProps {
    ...
    setFreelancerFeedback: React.Dispatch<React.SetStateAction<FreelancerFeedback[]>>;
...
const FreelancerPortal: React.FC<FreelancerPortalProps> = ({ accessId, teamMembers, projects, teamProjectPayments, teamPaymentRecords, rewardLedgerEntries, setFreelancerFeedback, showNotification, onUpdateRevision }) => {
    ...
    const tabs = [
        ...
        { id: 'feedback', label: 'Masukan', icon: MessageSquareIcon },
    ];
    ...
    const renderTabContent = () => {
        switch (activeTab) {
            ...
            case 'feedback': return <FeedbackTab freelancer={freelancer} setFreelancerFeedback={setFreelancerFeedback} showNotification={showNotification} />;
            default: return null;
        }
    }
...
const FeedbackTab: React.FC<{freelancer: TeamMember, setFreelancerFeedback: any, showNotification: any}> = ({ freelancer, setFreelancerFeedback, showNotification }) => {
    ...
    const [isSubmitting, setIsSubmitting] = useState(false);
     ...
     const handleFeedbackSubmit = (e: React.FormEvent) => {
        ...
        const newFeedback: FreelancerFeedback = { id: `FF-${Date.now()}`, freelancerId: freelancer.id, teamMemberName: freelancer.name, date: new Date().toISOString(), message: feedbackMessage };
        
        setTimeout(() => {
            setFreelancerFeedback((prev: any) => [newFeedback, ...prev]);
            showNotification('Saran Anda berhasil dikirim, terima kasih!');
            setFeedbackMessage(''); setIsSubmitting(false);
        }, 1000);
    };

    return (
         <div className="bg-brand-surface p-6 rounded-2xl shadow-lg border border-brand-border">
            <h3 className="text-xl font-bold text-brand-text-light mb-4">Kirim Masukan atau Saran</h3>
            <p className="text-sm text-brand-text-secondary mb-4">Punya saran untuk alur kerja, sistem, atau hal lain? Kami sangat menghargai masukan Anda untuk menjadi lebih baik.</p>
            <form onSubmit={handleFeedbackSubmit} className="space-y-4">
                <div className="input-group"><textarea id="feedbackMessage" value={feedbackMessage} onChange={e => setFeedbackMessage(e.target.value)} rows={5} className="input-field" placeholder=" " required/><label htmlFor="feedbackMessage" className="input-label">Sampaikan di sini...</label></div>
                <button type="submit" className="button-primary w-full" disabled={isSubmitting}>{isSubmitting ? 'Mengirim...' : 'Kirim Masukan'}</button>
            </form>
        </div>
    );
}
