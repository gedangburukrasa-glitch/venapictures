...
import { Project, ProjectStatus, PaymentStatus, TeamMember, Profile, AssignedTeamMember } from '../types';
import { ChevronLeftIcon, ChevronRightIcon, getProjectStatusColor, ClockIcon, UsersIcon, FileTextIcon, PlusIcon, MapPinIcon } from '../constants';
    ...
    const renderDetailView = () => (
        <div className="flex-1 flex flex-col">
            ...
            <div className="p-6 flex-1">
                 ...
                 <span className="text-xs font-medium px-2 py-0.5 rounded-full mt-2 inline-block" style={{ backgroundColor: profile.eventTypes.includes(selectedEvent?.projectType || '') ? eventTypeColors[selectedEvent?.projectType || '']+'30' : getProjectStatusColor(selectedEvent?.status || ProjectStatus.PENDING).replace('bg-','').replace('/20',''), color: profile.eventTypes.includes(selectedEvent?.projectType || '') ? eventTypeColors[selectedEvent?.projectType || ''] : getProjectStatusColor(selectedEvent?.status || ProjectStatus.PENDING).replace('bg-','').replace('/20','') }}>
    ...
    return (
        <div className="flex h-[calc(100vh-8rem)] bg-brand-surface rounded-2xl overflow-hidden">
            ...
            <div className="flex-1 flex flex-row overflow-hidden">
                                        ...
                                        <div key={i} onClick={() => openPanelForAdd(day)} className={`relative border-b border-l border-brand-border p-1.5 flex flex-col min-h-[120px] ${isCurrentMonth ? 'bg-brand-surface' : 'bg-brand-bg'} cursor-pointer hover:bg-brand-input transition-colors`}>
                                            ...
                                            <div className="flex-grow space-y-1 overflow-hidden">
                                                {events.map(event => {
                                                    ...
                                                    const bgColor = isInternal ? (eventTypeColors[event.projectType] || '#64748b') : getProjectStatusColor(event.status);
                                                    return (
                                            ...
                                            {eventsOnDate.map(event => {
                                                ...
                                                return (
                                                    <div key={event.id} className="relative mb-6">
                                                        ...
                                                        <div onClick={() => openPanelForEdit(event)} className="ml-4 p-4 bg-brand-bg rounded-lg cursor-pointer hover:bg-brand-input transition-colors">
                                                            <p className="font-semibold text-brand-text-light">{event.projectName}</p>
                                                            <p className="text-sm text-brand-text-secondary">{event.projectType}</p>
                                                            {event.team.length > 0 && (
                                                                <div className="flex items-center -space-x-2 mt-2">
                                                                    {event.team.map(t => <div key={t.memberId} className="w-6 h-6 rounded-full bg-brand-input flex items-center justify-center text-[10px] font-bold text-brand-text-secondary border-2 border-brand-bg" title={t.name}>{getInitials(t.name)}</div>)}
                                                                </div>
                                                            )}
                                                        </div>
                                                    </div>
                ...
                {/* Right Panel */}
                <div className={`flex-col flex-shrink-0 bg-brand-surface border-l border-brand-border transition-all duration-300 ease-in-out ${isPanelOpen ? 'w-96' : 'w-0'} overflow-hidden hidden lg:flex`}>
                    <div className="w-96 flex-1 overflow-y-auto">
                        {isPanelOpen && (
            ...
            <div className="lg:hidden">
                <Modal isOpen={isPanelOpen} onClose={() => setIsPanelOpen(false)} title={selectedEvent ? (panelMode === 'detail' ? `Detail Acara` : 'Edit Acara') : 'Buat Acara Baru'}>
                     {panelMode === 'detail' && selectedEvent ? renderDetailView() : renderForm()}
                </Modal>
            </div>
        </div>
