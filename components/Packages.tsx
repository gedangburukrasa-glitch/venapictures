    ...
    <div>
      ...
      <div className="grid lg:grid-cols-2 gap-8 items-start">
        ...
        <div className="bg-brand-surface p-6 rounded-2xl space-y-6">
            <h3 className="text-xl font-semibold text-brand-text-light border-b border-gray-700/50 pb-4">Paket Layanan</h3>
            
            {/* Package Form */}
            <div className="p-4 bg-brand-bg rounded-xl">
              <h4 className="text-lg font-semibold text-brand-text-light mb-4">{packageEditMode ? 'Edit Paket' : 'Tambah Paket Baru'}</h4>
              <form className="space-y-4" onSubmit={handlePackageSubmit}>
                  <div className="input-group">
                      <input type="text" id="packageName" name="name" value={packageFormData.name} onChange={handlePackageInputChange} className="input-field" placeholder=" " required/>
                      <label htmlFor="packageName" className="input-label">Nama Paket</label>
                  </div>
                  <div className="input-group">
                      <input type="number" id="packagePrice" name="price" value={packageFormData.price} onChange={handlePackageInputChange} className="input-field" placeholder=" " required/>
                      <label htmlFor="packagePrice" className="input-label">Harga (IDR)</label>
                  </div>
                  <div className="input-group">
                      <textarea id="description" rows={3} name="description" value={packageFormData.description} onChange={handlePackageInputChange} className="input-field" placeholder=" "></textarea>
            ...
            <div className="space-y-4 pt-4 border-t border-gray-700/50">
                {packages.map(pkg => (
                    <div key={pkg.id} className="p-4 bg-brand-bg rounded-xl">
                        <div className="flex justify-between items-start">
                            <div>
                                <h5 className="font-bold text-brand-text-light">{pkg.name}</h5>
                                <p className="text-sm text-brand-text-secondary mt-1">{pkg.description}</p>
                            </div>
                            <div className="text-right ml-4 flex-shrink-0">
                                <p className="text-base font-semibold text-brand-text-light whitespace-nowrap">{formatCurrency(pkg.price)}</p>
        ...
        <div className="bg-brand-surface p-6 rounded-2xl space-y-6">
            ...
            <div className="space-y-3 pt-4 border-t border-gray-700/50">
                {addOns.map(addOn => (
                    <div key={addOn.id} className="p-3 bg-brand-bg rounded-xl flex justify-between items-center">
